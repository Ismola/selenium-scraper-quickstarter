import logging
from flask import jsonify
from utils.streaming_service import streaming_service
from actions.web_driver import get_page, close_driver
from utils.config import STREAMING_ENABLED


def start_streaming_controller(url=None, browser='chrome'):
    """
    Controller to start streaming of browser automation
    
    Args:
        url (str): URL to navigate to (optional)
        browser (str): Browser type ('chrome' or 'firefox')
        
    Returns:
        dict: Response with streaming status and info
    """
    try:
        if not STREAMING_ENABLED:
            return {
                "success": False,
                "message": "Streaming is disabled in configuration",
                "error": "STREAMING_DISABLED"
            }
        
        if streaming_service.is_streaming():
            return {
                "success": False,
                "message": "Streaming is already active",
                "info": streaming_service.get_stream_info()
            }
        
        # Create new driver instance for streaming
        driver = get_page(browser=browser, url=url)
        
        if not driver:
            return {
                "success": False,
                "message": "Failed to create browser driver",
                "error": "DRIVER_CREATION_FAILED"
            }
        
        # Set driver for streaming service
        streaming_service.set_driver(driver)
        
        # Start streaming
        success = streaming_service.start_streaming()
        
        if success:
            logging.info(f"Streaming started successfully for URL: {url}")
            return {
                "success": True,
                "message": "Streaming started successfully",
                "info": streaming_service.get_stream_info(),
                "stream_url": "/stream/video",
                "viewer_url": "/stream/viewer"
            }
        else:
            close_driver(driver)
            return {
                "success": False,
                "message": "Failed to start streaming",
                "error": "STREAMING_START_FAILED"
            }
            
    except Exception as e:
        logging.error(f"Error starting streaming: {e}")
        return {
            "success": False,
            "message": f"Error starting streaming: {str(e)}",
            "error": "INTERNAL_ERROR"
        }


def stop_streaming_controller():
    """
    Controller to stop streaming
    
    Returns:
        dict: Response with operation status
    """
    try:
        if not streaming_service.is_streaming():
            return {
                "success": False,
                "message": "No active streaming session",
                "error": "NO_ACTIVE_STREAM"
            }
        
        # Stop streaming
        streaming_service.stop_streaming()
        
        # Close driver if exists
        if streaming_service.driver:
            close_driver(streaming_service.driver)
            streaming_service.driver = None
        
        logging.info("Streaming stopped successfully")
        return {
            "success": True,
            "message": "Streaming stopped successfully"
        }
        
    except Exception as e:
        logging.error(f"Error stopping streaming: {e}")
        return {
            "success": False,
            "message": f"Error stopping streaming: {str(e)}",
            "error": "INTERNAL_ERROR"
        }


def get_streaming_status_controller():
    """
    Controller to get current streaming status
    
    Returns:
        dict: Current streaming status and information
    """
    try:
        return {
            "success": True,
            "streaming_enabled": STREAMING_ENABLED,
            "info": streaming_service.get_stream_info()
        }
        
    except Exception as e:
        logging.error(f"Error getting streaming status: {e}")
        return {
            "success": False,
            "message": f"Error getting streaming status: {str(e)}",
            "error": "INTERNAL_ERROR"
        }


def navigate_streaming_controller(url):
    """
    Controller to navigate to a new URL during streaming
    
    Args:
        url (str): URL to navigate to
        
    Returns:
        dict: Response with navigation status
    """
    try:
        if not streaming_service.is_streaming():
            return {
                "success": False,
                "message": "No active streaming session",
                "error": "NO_ACTIVE_STREAM"
            }
        
        if not streaming_service.driver:
            return {
                "success": False,
                "message": "No driver available",
                "error": "NO_DRIVER"
            }
        
        # Navigate to new URL
        streaming_service.driver.get(url)
        
        logging.info(f"Navigated to: {url}")
        return {
            "success": True,
            "message": f"Successfully navigated to {url}",
            "current_url": streaming_service.driver.current_url
        }
        
    except Exception as e:
        logging.error(f"Error navigating during streaming: {e}")
        return {
            "success": False,
            "message": f"Error navigating: {str(e)}",
            "error": "NAVIGATION_ERROR"
        }


def execute_streaming_action_controller(action_type, **kwargs):
    """
    Controller to execute actions during streaming (click, type, etc.)
    
    Args:
        action_type (str): Type of action ('click', 'type', 'scroll', etc.)
        **kwargs: Action-specific parameters
        
    Returns:
        dict: Response with action execution status
    """
    try:
        if not streaming_service.is_streaming():
            return {
                "success": False,
                "message": "No active streaming session",
                "error": "NO_ACTIVE_STREAM"
            }
        
        if not streaming_service.driver:
            return {
                "success": False,
                "message": "No driver available",
                "error": "NO_DRIVER"
            }
        
        driver = streaming_service.driver
        
        if action_type == "click":
            from selenium.webdriver.common.by import By
            locator = kwargs.get('locator')
            value = kwargs.get('value')
            
            if locator and value:
                element = driver.find_element(getattr(By, locator.upper()), value)
                element.click()
                
        elif action_type == "type":
            from selenium.webdriver.common.by import By
            locator = kwargs.get('locator')
            value = kwargs.get('value')
            text = kwargs.get('text', '')
            
            if locator and value:
                element = driver.find_element(getattr(By, locator.upper()), value)
                element.clear()
                element.send_keys(text)
                
        elif action_type == "scroll":
            x = kwargs.get('x', 0)
            y = kwargs.get('y', 500)
            driver.execute_script(f"window.scrollBy({x}, {y});")
            
        elif action_type == "execute_script":
            script = kwargs.get('script', '')
            if script:
                result = driver.execute_script(script)
                return {
                    "success": True,
                    "message": "Script executed successfully",
                    "result": result
                }
        
        logging.info(f"Executed action: {action_type}")
        return {
            "success": True,
            "message": f"Action '{action_type}' executed successfully"
        }
        
    except Exception as e:
        logging.error(f"Error executing action during streaming: {e}")
        return {
            "success": False,
            "message": f"Error executing action: {str(e)}",
            "error": "ACTION_EXECUTION_ERROR"
        }